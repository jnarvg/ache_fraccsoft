...
<div class="content mt-3">
  <div class="card collapse" id="filtros">
    <div class="card-body">
      {!! Form::open(array('route'=>'v_pagando', 'method'=>'get', 'autocomplete'=>'off')) !!}
        <div class="row">
          <div class="col-md-3">
          ...
          <div class="col-md-3">
            <div class="form-group">
              ...
              <select class="form-control" placeholder="Estatus" name="estatus_bs[]" id="estatus_bs" multiple="multiple">
                <option selected="true" value="7">Pagando</option>
              </select>
            </div>
          </div>
          <div class="col-md-3">
      ...
      <div class="table-responsive ">
        <table class="table table-hover table-withoutborder text-sm">
          ...
          <tbody>
            ...
            <tr>
              <td class="center">
                <a class="text-dark" href="{{URL::action('ProspectosController@show', [ $prospecto->id_prospecto, 'v_pagando' ])}}" >
                {{ $prospecto->nombre}}</a>
              </td>
              <td class="center">
                <a class="text-dark text-sm" href="{{URL::action('ProspectosController@show', [ $prospecto->id_prospecto, 'v_pagando' ])}}" >
                {{ $prospecto->correo}}</a>
              </td>
              <td class="center">
                <a class="text-dark" href="{{URL::action('ProspectosController@show', [ $prospecto->id_prospecto, 'v_pagando' ])}}" >
                  @if ($prospecto->fecha_ultimo_pago != null)
                    {{ date('Y-m-d',strtotime($prospecto->fecha_ultimo_pago)) }}
                  @endif
                </a>
              </td>
              <td class="center">
                <a class="text-dark" href="{{URL::action('ProspectosController@show', [ $prospecto->id_prospecto, 'v_pagando' ])}}" >
                  @if ($prospecto->monto_ultimo_pago != null)
                    $ {{ number_format($prospecto->monto_ultimo_pago , 2 , "." , ",") }}
                  @else
                    $0.00
                  @endif
                  </a>
              ...
              <td class="center">
                <a class="text-dark" href="{{URL::action('ProspectosController@show', [ $prospecto->id_prospecto, 'v_pagando' ])}}" >
                  @if ($prospecto->pagado != null)
                    $ {{ number_format($prospecto->pagado , 2 , "." , ",") }}
                  @else
                    $0.00
                  @endif
                </a>
              ...
              <td class="center-acciones">
                <a href="{{URL::action('ProspectosController@show', [ $prospecto->id_prospecto, 'v_pagando' ])}}" ><button class="btn-ico" ><i class="fas fa-pencil-alt"></i></button></a>
                @if (auth()->user()->id == $prospecto->asesor_id)
                <a href="#" data-target="#modal-delete{{$prospecto->id_prospecto}}" data-toggle="modal"><button class="btn-ico" ><i class="fas fa-trash-alt"></i></button></a>               
                @endif
              </td>
            </tr>
